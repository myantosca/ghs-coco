    ...
    \paragraph{}{
      \begin{algorithm}
        ...
        \begin{algorithmic}
          ...
          \REQUIRE{$\textsc{Machine-Hash}(v) = ((h_a * v + h_b) \mod M_{61}) \mod k$}
          \IF{$r = 0$}{
          ...
          \FOR{$i \in [0, |E|_k-1]$}{
            ...
            \STATE{$machine_u \gets \textsc{Machine-Hash}(e_{uv}[0])$}
            \STATE{$machine_v \gets \textsc{Machine-Hash}(e_{vu}[0])$}
            \STATE{Add $e_{uv}$ to the exchange buffer for $machine_u$.}
            \STATE{Add $e_{vu}$ to the exchange buffer for $machine_v$.}
          }\ENDFOR
          \STATE{Exchange edge buffers via \textsc{GatherV} operation.}
        \end{algorithmic}
      \end{algorithm}
    }
    \paragraph{}{
