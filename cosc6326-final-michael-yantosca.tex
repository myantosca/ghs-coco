...
\section{Analysis}{
  \subsection{\texttt{ghs-coco}}{
    ...
    \paragraph{}{
      \begin{algorithm}
        ...
        \begin{algorithmic}
          ...
          \REQUIRE{$|R_r|$, the number of nodes where $|N| > 0$}
          \FORALL{$t \in T_r$}{
            \IF{$M(t) = \emptyset$ and $N(t) = \emptyset$}{
              \STATE{$s(t) \gets \textsc{test}$}
              \STATE{$S_r \gets S_r \cup \{ t \}$}
              \STATE{$|R_r| \gets |R_r| - 1$}
            }\ELSE{
              \STATE{$T_0 \gets T_0 \cup \{ t \}$}
              \STATE{$T_r \gets T_r \setminus \{ t \}$}
            }\ENDIF
          }\ENDFOR
          \STATE{$\omega_e \gets |V_e| = |T_r|$}
        ...
        \begin{algorithmic}
          \FORALL{$u \in S_r$}{
              \IF{$s(u) = \textsc{test}$}{
                ...
                \IF{$i_g(u) \neq i(u)$}{
                  \STATE{$send(X_S)[r] \gets send(X_S)[r] + ( \textsc{done}, p(v), u_e, v_e )$}
        ...
        \begin{algorithmic}
            \FORALL{$(q, i_v, a, b) \in recv(X_S)$}{
              ...
              \IF{$q = \textsc{ping}$}{
                ...
                \STATE{$|V_\omega| = |V_\omega| - 1$}
                \STATE{$s(v) = \textsc{mwoe}$}
                \STATE{$S_r \gets S_r \cup \{ v \}$}
                \IF{$u_e(v) = i(v)$}{
                  \STATE{$V_e \gets V_e \cup \{ v \}$}
                }\ENDIF
              }\ENDIF              
            }\ENDFOR
        \end{algorithmic}
    ...
    \paragraph{$q = \textsc{mwoe}$}{
      ...
      merging set $V_e$ to participate in the merging phase.
    }
    ...
    \paragraph{}{
      \begin{algorithm}
        ...
        \begin{algorithmic}
          \STATE{$V_F \gets \emptyset$}
          \FORALL{$u \in V_e$}{
            \STATE{$m_v \gets \textsc{Machine-Hash}(h_a, h_b, k,v_e(u))$}
            \STATE{$send(X_S)[m_v] \gets send(X_S)[m_v] + ( \textsc{join}, v_e(u), i(u) )$}
          }\ENDFOR
          ...
          \STATE{$|V_N| \gets |V_r| - |V_F| - |T_0|$}
          \STATE{$\omega_F \gets |V_N| = 0$}
